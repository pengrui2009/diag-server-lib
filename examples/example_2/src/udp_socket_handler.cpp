...
DoipUdpSocketHandler::DoipUdpSocketHandler(kDoip_String &udp_ip_address, uint16_t udp_port_num, 
    PortType port_type, UdpMessageFunctor udp_handler)
    : local_ip_address_{udp_ip_address}, 
      local_port_num_{udp_port_num},
      port_type_{port_type} {
  // create sockets and start receiving
  if (port_type == UdpSocket::PortType::kUdp_Broadcast) {
    udp_socket_ =
        std::make_unique<UdpSocket>(local_ip_address_, local_port_num_, port_type_,
